...
const Player = ({ player, handleAddToCart }) => {
  const { id, name, age, position, club, nationality, price, trophy, picture } =
    player;
  return (
    <div className="flex items-center static md:sticky md:top-20 gap-2 bg-slate-100 shadow-md rounded-md">
      <figure className="">
        <img
          className="w-64 object-top rounded-l-md h-64 object-cover"
          src={picture}
          alt=""
        />
      </figure>
      <div className="flex-1 px-3">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between mt-3">
          ...
          <button
            onClick={() =>
              handleAddToCart({ id, name, price, position, picture })
            }
            className="flex items-center gap-2 bg-yellow-300 text-md text-slate-800 text-sm font-semibold px-3 py-2 rounded-lg"
          >
            <ShoppingCartIcon className="w-6 h-6"></ShoppingCartIcon> Add To
            Cart
          </button>
        </div>
      </div>
